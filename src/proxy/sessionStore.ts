...
// renameSync — a new inode — so identity-keyed caching is exact. It takes a
// ~135 ms synchronous parse of a large store off every lookup, including out
// from under the session lifecycle lock. Callers must treat the returned
...
function writeStore(path: string, document: SessionStoreDocument): void {
  ...
  try {
    ...
    // of the bytes and of the stringify CPU (72 ms → 48 ms measured).
