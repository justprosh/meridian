...
export async function acquireActiveTranscriptLease(
  ...
  await withSidecarLock(options, async (paths) => {
    ...
    const unarmedLeaseTtlMs = nonNegativeOption(options.preparedGraceMs, DEFAULT_PREPARED_GRACE_MS, "preparedGraceMs")
...
export async function reconcile(
  ...
  return withSidecarLock(options, async (paths) => {
    ...
    for (const resource of Object.values(sidecar.resources)) {
      if (pruneDeadActiveLeases(resource, now, preparedGraceMs)) changed = true
    }
...
async function writeSidecar(path: string, sidecar: SessionGcSidecar): Promise<void> {
  ...
  try {
    ...
    // under the lock: 1610 KiB pretty vs 1216 KiB compact on the deployment).
...
function pruneDeadActiveLeases(resource: TranscriptResource, now: number, unarmedLeaseTtlMs: number): boolean {
  ...
  for (const [token, lease] of Object.entries(resource.activeLeases)) {
    ...
    // legitimately holds one longer than the turn watchdog (the prepared grace
    // passed here), so an unarmed lease older than that — writer or publication,
    // with its owner still alive — can only be a release that failed; age retires it.
