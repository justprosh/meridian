 ...
 *
 * Text alone is not always an answer either. Observed live under the `.4`
 * guard: on a deny-boundary continuation (a session's second turn, cold cache)
 * the model produced an empty thinking block plus one short text that only
 * ANNOUNCED the work — "Let me read the config…" — and ended the turn with
