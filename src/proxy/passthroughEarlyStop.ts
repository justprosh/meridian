 ...
 * same state an early stop leaves, and for the same reason: the query is cut
 * before the turn completes. Resuming from that tail makes the SDK synthesize
 * a "Continue from where you left off." turn, which the model answers with
 * meta text ("No response requested.") or an empty turn. Each empty turn then
 * becomes the next tail, so the session never recovers on its own: every
 * following turn comes back empty until the lineage is discarded.
 *
